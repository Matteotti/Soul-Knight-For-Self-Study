...
public class SwitchWeapon : MonoBehaviour
{
    ...
    private void Update()
    {
        if (nowWeaponName != null)
            nowWeaponName.transform.position = Camera.main.WorldToScreenPoint(transform.parent.position) + new Vector3(deltaXForName, deltaYForName, 0);
    ...
    {
        nowWeaponOnGround = Instantiate(weaponOnGround, transform.position + new Vector3(deltaXForWeapon, deltaYForWeapon, 0), Quaternion.identity, weaponFather);
        nowWeaponOnGround.GetComponent<WeaponOnGround>().thisWeapon = this.GetComponent<Fire>().weaponHeld;
        nowWeaponOnGround.GetComponent<SpriteRenderer>().sprite = this.GetComponent<Fire>().weaponHeld.weaponImage;
        ...
        weaponUIImage.SetNativeSize();
        if (nowWeaponName == null)
            nowWeaponName = Instantiate(weaponName, Camera.main.WorldToScreenPoint(transform.parent.position) + new Vector3(deltaXForName, deltaYForName, 0), Quaternion.identity, canvas.transform);
    ...
    {
        this.GetComponent<Fire>().CancleFullShoot();
        this.GetComponent<Fire>().knifeSpin = false;
        if (this.GetComponent<Fire>().weaponHeld.type == Weapon.WeaponType.bow || weapon.type == Weapon.WeaponType.bow)
            this.GetComponent<Fire>().allowFireArrow = false;
        this.GetComponent<Fire>().weaponHeld = weapon;
        this.GetComponent<Animator>().runtimeAnimatorController = weapon.weaponAnimator;
        this.GetComponent<SpriteRenderer>().sprite = weapon.weaponImage;
        weaponUIImage.sprite = weapon.weaponImage;
