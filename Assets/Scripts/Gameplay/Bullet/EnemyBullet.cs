...
public class EnemyBullet : MonoBehaviour
{
    ...
    public float sinCosSpeed;
    public float sinCosChangeSpeed;
    public float sinCosAngle;
    ...
    private void Update()
    {
        ...
        if (trackPlayer && Time.frameCount % 10 == 0 && Time.frameCount > 0.5 / Time.deltaTime)
        {
            ...
            deltaUp = (transform.eulerAngles.z < angle) ? (angle - transform.eulerAngles.z) : (angle + (360 - transform.eulerAngles.z));//ÄæÊ±Õë
            deltaDown = 360 - deltaUp;//Ë³Ê±Õë
        ...
        if(thisEnemyBulletType == enemyBulletType.graphicSinCos)
        {
            ...
            rb.velocity = (bulletSpeed * right + Mathf.Cos(sinCosAngle) * up) * sinCosSpeed;
        }
    }
